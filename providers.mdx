...
<Steps>
  ...
  <Step title="select a model">
    recommended models:
    - `gpt-oss-120b` — best quality, 128k context
    - `qwq-32b` — fast reasoning model
...
<Steps>
  ...
  <Step title="select a model">
    ...
    - `gemini-1.5-flash` — reliable and efficient
  </Step>
</Steps>
...
— (4) local with lm studio (unlimited, private)
...
|----------|-------|-------------------|
| **openai** | paste your key, select provider "openai" | `gpt-5-nano` (cheapest), `gpt-4o-mini`, `gpt-4o` |
| **anthropic** | paste your key, select provider "anthropic" | `claude-sonnet-4-20250514`, `claude-3-haiku` |

— (6) which provider should i use?

| your situation | recommended provider |
|----------------|---------------------|
| just want it to work for free | **cerebras** with `gpt-oss-120b` |
...
| google ecosystem user | **google gemini** |
